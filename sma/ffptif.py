...
def ffp_tif(filename,xmlname):    
    print ("ffptif started at " + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + " on file: " + filename)
    ...
    print ("xcrop: %d. ycrop: %d for bk size %d" %(xcrop,ycrop,bksize))
    ...
    if par.d3peaks == 1:
        ...
        for i in range(0,no_sets): #goes from 0 to no_sets -1
            ...
            fr_bk = smbkgr.sm_bkgr(medimg,bksize) #seems okay; check once showing images added
            medimg = medimg-fr_bk
            ...
            sliceresult = ffpslice.ffp_slice(medimg,currset_st,par) #these are all 'raw' results in medimg frame. Wait until the very end to map, if needed   This is the only piece that still requires Par
        ...
        hbinnum = int(np.round((p95 - times.min())/par.frameset,0))
    ...
    if par.autocont==1:
        print ('automatically calling aptif')
        #print 'apdax.py'
        from aptif import ap_tif
